...
<head>
    <meta charset="UTF-8">
    <title>Sign in | Sunshine Coast Film Club</title>
...
if (isset($_POST['signUpSubmit'])) {
    ...
    if (empty($username) || empty($email) || empty($password) || empty($password_confirm)) {
        ...
        // If the POST data is invalid, redirect to the sign up page
        header("Location: ../sign-up/index.php?error=invalidemail&username=" . $username);
        exit();
        ...
        // If the POST data is invalid, redirect to the sign up page
        header("Location: ../sign-up/index.php?error=invalidusername&email=" . $email);
        exit();
    } else if ($password !== $password_confirm) {
        // If the POST data is invalid, redirect to the sign up page
        header("Location: ../sign-up/index.php?error=passwordcheck&username=" . $username . "&email=" . $email);
        exit();
    } else {
        ...
        $result = $mysqli->query($sql);
            if ($row = mysqli_fetch_assoc($result)) {
                ...
                header("Location: ../sign-up/index.php?error=userexists&username=" . $username . "&email=" . $email);
                exit();
            } else {

                $filebasename = basename($_FILES["photo"]["name"]);
                $filepath = $_FILES["photo"]["name"];

                require('../partials/upload-profile.php');

                if ($uploadOk == 0) {
                    echo "There was an error with your file upload. Please try again. Error code: ";
                    die;
                    exit;
                } else {
                    $userProfileFileName = $newFileName;
                }
                ...
                $sql = "INSERT INTO users (username, email, password, profileImage) VALUES ('" . $username . "', '" . $email . "', '" . $hashedPwd . "', '"  . $userProfileFileName . "')";
                $result = $mysqli->query($sql);
