            ...
            <div class="bg-white h-full dark:bg-gray-700">
                <div class="flex items-center justify-start pt-16 ml-5 mb-16">
                    <div class="pr-5">
                        ...
                        <h2 class="text-sm text-orange-700 font-semibold tracking-wide uppercase absolute dark:text-gray-300 top6.5 left-5">Admin Dashboard</h2>
                        </div>
                    </div>
                </div>
                <nav class="mt-6">
        ...
        <div class="flex flex-col w-full md:space-y-4">
            ...
            <div class="overflow-auto h-screen pb-24 px-4 md:px-6">
                <h1 class="text-4xl font-semibold text-gray-800 dark:text-white">
                    Welcome to the admin dashboard, <span class="text-4xl font-semibold text-orange-700 dark:text-gray-400"><?php echo $_SESSION["nickname"]?></span>
                </h1>
                    ...
                    <div class="flex items-center w-full space-x-4">
                        ...
                        <div class="w-full">
                            <div class="shadow-lg px-4 py-6 w-full bg-white dark:bg-gray-700 relative">
                                ...
                                <?php 
                                    require('../connectDB.php');
                                    $sql = "SELECT COUNT(reportID) AS reportCount FROM reports;";
                                    $result = $mysqli->query($sql);
                                    $row = mysqli_fetch_assoc($result);
                        ...
                        <div class="w-full">
                            <div class="shadow-lg px-4 py-6 w-full bg-white dark:bg-gray-700 relative">
                                ...
                                <?php 
                                    require('../connectDB.php');
                                    $sql = "SELECT COUNT(nickname) AS userCount FROM users;";
                                    $result = $mysqli->query($sql);
                                    $row = mysqli_fetch_assoc($result);
                                    echo $row["userCount"];
                                    $mysqli->close();
                ...
                <?php if (!$_GET["report"]) { require('../partials/dashboard-listreports.php'); } else if ($_GET["report"] == "true") { require('../partials/dashboard-report.php'); } else { require('../partials/dashboard-dogparks.php'); }  ?>
            </div>
        </div>
    </div>
</main>
