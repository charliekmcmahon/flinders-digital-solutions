...
if (isset($_POST["user-submit"])) {
    ...
    echo($firstName.$lastName.$birthdate.$licenseCRN.$state.$gender.$address.$vehicleType.$vehiclePlate);
    ...
    if ($isUserInDB < 1) {
        ...
        $updateSQL = "INSERT INTO `tblclientdata` (`firstName`, `lastName`, `dateOfBirth`, `licenseUUID`, `gender`, `address`, `vehicleType`, `vehiclePlate`, `email`) VALUES ('". $firstName ."', '". $lastName ."', '". $birthdate ."', '". $licenceUUID ."', '". $gender ."', '". $address ."', '". $vehicleType ."', '". $vehiclePlate ."', '". $email . "')";
        echo "<br>" . $updateSQL;
        runQuery($updateSQL);
    ...
    // Pass data through
    $passthrough = true;
    $firstName = filter_var($_POST["old-firstName"], FILTER_SANITIZE_STRING);
    $licenseCRN = filter_var($_POST["old-licenseNumber"], FILTER_SANITIZE_NUMBER_INT);
    $state = filter_var($_POST["old-state"], FILTER_SANITIZE_STRING);
    $email = filter_var($_POST["old-email"], FILTER_SANITIZE_EMAIL);
...
<body>
    ...
    <form action="update_account.php" method="POST">
        <button class="w-100 mb-2 btn btn-lg rounded-4 btn-primary" type="submit" name="user-update">El update</button>
    </form>
    <section>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content rounded-5 shadow">
                    <div class="modal-header p-5 pb-4 border-bottom-0">
                        <h2 class="fw-bold mb-0">Update your details here</h2>
                    </div>
                    ...
                    <div class="modal-body p-5 pt-0">
                        <form class="" action="update-account.php" method="POST">
                            ...
                            <div class="form-floating mb-3">
                                <input type="text" class="form-control rounded-4" id="floatingLicenseNumber"  name="licenseNumber" <?php if ($passthrough == true) { echo 'value="'.$licenseCRN.'"';} else { echo "error";} ?> placeholder="101 202 303" <?php if ($passthrough == true) { echo "disabled";} ?>>
                                <label for="floatingLicenseNumber">License Number (CRN)</label>
                            </div>
                            <div class="form-floating mb-3">
                                <select class="form-select"  name="state" <?php if ($passthrough == true) { echo `value="${$state}"`;} ?> id="floatingStateSelect" <?php if ($passthrough == true) { echo "disabled";} ?>>
