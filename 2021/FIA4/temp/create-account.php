...
if (isset($_POST["user-submit"])) {
    ...
    $userInDB = runQuery(("SELECT * FROM `tblclientdata` WHERE `licenseUUID` = '" . $licenceUUID . "'"));
    
    echo "<br> working";

    if (!isset($userInDB) || $userInDB == true) {
        // User is not already in the DB, INSERT them
        $insertSQL = "INSERT INTO `tblclientdata` (`firstName`, `lastName`, `dateOfBirth`, `licenseUUID`, `gender`, `address`, `vehicleType`, `vehiclePlate`) VALUES ('". $firstName ."', '". $lastName ."', '". $birthdate ."', '". $licenceUUID ."', '". $gender ."', '". $address ."', '". $vehicleType ."', '". $vehiclePlate ."')";
        echo "<br>" . $insertSQL;
        runQuery($insertSQL);
